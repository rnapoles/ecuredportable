...
bool DatabaseManager::isCompressed()
{
    if(db.isOpen()){
        QString strQuery =  "SELECT * FROM wikitext limit 1;";
        ...
        while (query.next()) {
            ...
            //qDebug()<<" Result: " << value.toHex().left(4);
            if(value.toHex().left(4) == QByteArray("78da")) return true;
        }

        if(query.lastError().type() != QSqlError::NoError){
            ErrorStr = query.lastError().text();
            emit Error(ErrBd,"Error al comprobar si la fuente documental se encuentra comprimida.",ErrorStr);
            return false;
        }
...
//    }
    ...
    this->p_IsTableAvailable=false;
    this->p_NumArts=NumArts;
    this->p_NumFields= NumFields;
    */
    this->DbInfoAvailable = true;
