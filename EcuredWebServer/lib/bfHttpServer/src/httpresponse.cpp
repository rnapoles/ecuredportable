...
HttpResponse::HttpResponse(QTcpSocket* socket, QObject *parent):QObject(parent) {
    this->socket=socket;
    statusCode=200;
    statusText="OK";
    sentHeaders=false;
...
void HttpResponse::write(QString data, bool lastPart, int s){
    this->write(data.toUtf8(),lastPart);
}
